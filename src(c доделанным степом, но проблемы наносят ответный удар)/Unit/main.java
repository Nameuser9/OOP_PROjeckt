...
public  class main {
    ...
    public static void add_army1(ArrayList<units> army , int lead) {//не происходит полного создания команд, при запуске создаёт то 6, то 7, то 9 объектов сейчас не знаю что с этим делать, но буду доводить до ума
        //не могу понять в чём причина ошибки, можете пожалуйста подсказать
        for (int i = 0; i < 10; i++) {
            int b;
            ...
            if (a == 1) {
                ...
                b =lead/5;
                army.add(new  peasent(1,i+1,b));//ПРОБЛЕМА попытка деления на 0(а почему?) ведь лидерство величина статическая и имеется у каждого класса отдельно
...
//                System.out.println(wizzard.get_Info());
        ...
        public static void add_army2(ArrayList<units> army, int lead) {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                int a = new Random().nextInt(5);
                if (a == 1) {
                    b =lead/peasent.leadership;
                    army.add(new  peasent(10,i+1,b));
//                    army.add(peasent);
//                    System.out.println(peasent.get_Info());
                ...
                if (a == 2) {
                    b =lead/spearman.leadership;
                    army.add(new  spearman(10,i+1 ,b));
//                    army.add(spearman);
//                    System.out.println(spearman.get_Info());

                }
                if (a == 3) {
                    b =lead/crossbower.leadership;
                    army.add(new  crossbower(10,i+1 ,b));
//                    army.add(crossbower);
//                    System.out.println(crossbower.get_Info());

                }
                if (a == 4) {
                    b =lead/cleric.leadership;
                    army.add(new  cleric(10,i+1 ,b));
//                    army.add(cleric);
